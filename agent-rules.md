...
**Dani는 ACP 세션용 Discord 스레드만 생성하고 대기한다.**

- ❌ `sessions_spawn`에 `task` 파라미터 넣지 않음
- ❌ 초기 지시 던지지 않음
- ✅ 스레드만 열고 Eli에게 알림
- ✅ Eli가 해당 스레드에서 task를 던지고 Codex 진행 관리

이 규칙은 agent-rules.md 문서에도 명시되어 있으며, 향후 모든 ACP 세션에서 적용된다.
